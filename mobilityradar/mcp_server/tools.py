...
from mobilityradar.nl_query import parse_query
...
def handle_route_suggest(*, db_path: Path, days: int = 7, limit: int = 10) -> str:
    """Suggest route-related insights from recent mobility articles."""
    cutoff = datetime.now(tz=UTC) - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        rows = conn.execute(
            "SELECT title, source, entities_json, collected_at FROM articles "
    ...
    for title, source, entities_json, collected_at in rows:
        ...
        if any(keyword in title_lower for keyword in mobility_keywords):
            entity_info = ""
            if entities_json:
                try:
                    entities = json.loads(entities_json)
                    if entities:
                        entity_info = f" [{', '.join(entities.keys())}]"
                except json.JSONDecodeError:
                    pass
            insights.append(f"- {title}{entity_info} ({source}, {collected_at})")

    if not insights:
        return "No route-related insights found."
    return f"Route insights ({len(insights)} item(s) in last {days} days):\n" + "\n".join(
        insights[:limit]
    )
