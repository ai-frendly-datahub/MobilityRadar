...
from mobilityradar.mcp_server.tools import (
    handle_recent_updates,
    handle_route_suggest,
    handle_search,
    handle_sql,
    handle_top_trends,
)
...
def _list_tool_specs() -> list[dict[str, object]]:
    return [
        ...
        {
            "name": "route_suggest",
            "description": "Suggest route-related insights from recent mobility articles.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "days": {"type": "integer", "minimum": 1},
...
def _call_tool_handler(name: str, arguments: object) -> str:
    ...
    if name == "route_suggest":
        return handle_route_suggest(
            db_path=_db_path(),
            days=_as_int(args.get("days"), 7),
            limit=_as_int(args.get("limit"), 10),
        )
