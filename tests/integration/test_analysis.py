...
def test_entity_extraction_integration(
    ...
    assert "bike_sharing" in article_1.matched_entities
    assert "자전거" in article_1.matched_entities["bike_sharing"]

    article_2 = analyzed[1]
    assert "electric_vehicle" in article_2.matched_entities
    assert "전기차" in article_2.matched_entities["electric_vehicle"]

    article_3 = analyzed[2]
    assert "kickboard" in article_3.matched_entities
    assert "킥보드" in article_3.matched_entities["kickboard"]

    article_4 = analyzed[3]
    assert "public_transit" in article_4.matched_entities
    assert "버스" in article_4.matched_entities["public_transit"]

    article_5 = analyzed[4]
    assert "micro_mobility" in article_5.matched_entities
    assert "스쿠터" in article_5.matched_entities["micro_mobility"]
